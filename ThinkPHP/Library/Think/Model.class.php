...
class Model {
    ...
    protected function count($pk = '') {
        return $this->field($pk ? $pk : $this->pk)->limit(0, C('DB_LIMIT'))->_getRow();
    }
     ...
     * @return boolean|int
     */
    private function _getRow($options = array()) {
        $pk = $this->getPk();
    ...
    /**
     * 单条更新（如果数据不存在就新增）
     * @param type $data需要更新的数据（一维数组）
     * @param type $field （需要更新的字段默认根据数组列读取 提高效率建议设置除主键以外的字段）
     * @return boolean
     */
    public function saveOne($data, $field = "") {
    ...
    public function saveAll($data, $field = "") {
        ...
        foreach ($data as $k => $v) {
            $v = $this->_facade($v);
            if (is_array($v)) {
                $strdata.=$strdata ? ',' . $this->_parsargs($v) : $this->_parsargs($v);
            } else {
                $strdata = $this->_parsargs($v); // "('" . implode("','", $data) . "')";
                break;
            }
        }
        $field || $field = $keys;
    ...
    public function saveUpdate($data) {
        ...
        foreach ($keys as $k => $v) {
            ...
            $sql.=$this->_parssql($data, $v, $pk, $where);
        }
        $sql = trim($sql, ',');
        $sql .= ' where ' . $pk . ' IN (' . $where . ')';
     ...
     * @param type $value
     */
    protected function _parssql($data, $k, $pk, &$where) {
        $sql = $k . '= CASE ' . $pk;
        foreach ($data as $key => $value) {
            $where .= $where ? ',' . $value[$pk] : $value[$pk];
            $sql.=' WHEN ' . $value[$pk] . ' THEN ';
            $val = $value[$k];
            if (is_array($val)) {
                switch ($val[0]) {
                    case 'function':
     ...
     * @param type $data
     */
    protected function _parsargs($data) {
        $strdata = "(";
        foreach ($data as $key => $value) {
            if (is_array($value)) {
                switch ($value[0]) {
    ...
    public function regex($value, $rule) {
        $validate = array(
            ...
            'double' => '/^[-\+]?\d+(\.\d+)?$/',
            'english' => '/^[A-Za-z]+$/',
            "mobile" => "/^(13[0-9]|14[0-9]|15[0-9]|16[0-9]|18[0-9]|17[0-9])\d{8}$/",
        );
        // 检查是否有内置的正则表达式
