...
class Common_util_pub {
     ...
     * 	作用：生成签名
     */
    public function getSign($Obj, $fag = false) {
        foreach ($Obj as $k => $v) {
            $Parameters[$k] = $v;
        }
        //签名步骤一：按字典序排序参数
        ...
        //签名步骤三：MD5加密
        $fag ? $String = sha1($String) : $String = md5($String);
    ...
    function postXmlSSLCurl($xml, $url, $second = 30) {
        $str = dirname(__FILE__) . WxPayConf_pub::SSLCERT_PATH;
        $ch = curl_init();
        //超时时间
...
class Wxpay_client_pub extends Common_util_pub {
     ...
     * 	作用：使用证书post请求xml
     */
    function postXmlSSL($is_server = false) {

        $xml = $is_server ? $this->createXmlServer() : $this->createXml();
