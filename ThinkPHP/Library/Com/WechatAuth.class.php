...
class WechatAuth {
    ...
    public function getAccessToken($type = 'client', $code = null) {
        ...
        if (is_array($token)) {
            if (isset($token['errcode'])) {
                ...
                $this->accessToken = $token['access_token'];
                $token['time'] = NOW_TIME + $token['expires_in'] - 60;
                S('accessToken' . $type, $token);
                return $token;
            }
        } else {
    ...
    public function getjsapi_ticket() {
        ...
        $time = NOW_TIME;
        if (!empty($token) && $token['time'] > $time) {
            return $this->_jsapi_ticket = $token['ticket'];
        }
        ...
        if (is_array($token)) {
            if (isset($token['errcode']) && $token['errcode'] == 0) {
                S("sapi_ticket", ['ticket' => $token['ticket'], "time" => NOW_TIME + $token['expires_in'] - 60]);
                return $this->_jsapi_ticket = $token['ticket'];
            } else {
                throw new \Exception($token['errmsg']);
            }
     ...
     * @return type
     */
    public function download($media_id, &$httpCode) {
        $url = $this->mediaGet($media_id); //获取下载地址
        $ch = curl_init();
     ...
     * 获取openid
     */
    public function getopenid($url, $state = "123") {
        if (empty($_GET ['state'])) {
            $param = [
                'appid' => $this->appId,
                'redirect_uri' => $url,
            ...
            $url = $this->requestCodeURL . '?' . http_build_query($param) . '#wechat_redirect';
            redirect($url);
        } else {
            $param = [
                'appid' => $this->appId,
