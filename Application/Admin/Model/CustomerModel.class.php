...
class CustomerModel extends Model {
	...
	public function get_lists($data, $page = 0, $row = 0){
		...
		if($data['waters_type'] == 1){
			//公海
			$users_fields_model = M('waters_users_fields');
		}else{
			$users_fields_model = M('users_fields');
		}
		$users_fields_data = $users_fields_model->field('fields_text,sort')->where(array('users_id'=>$users_id,'status'=>1))->order('sort asc')->select();
		...
		$fields = 'c.id,c.name,c.mobile,c.type,c.follow_status,c.next_follow_time,c.users_id,c.person_id,c.create_time,c.update_time,c.no_follow_day';
	...
	public function get_screen($data){
		...
		if($data['waters_type'] == 1){
			//公海
			$model = M('waters_users_fields');
			$screen_model = M('waters_screen_fields');
		}else{
			$model = M('users_fields');
			$screen_model = M('screen_fields');
		}
		//获取显示列字段
		$show_data = array();
		$show_data[] = array('fields_text'=>'name','title'=>'客户名称','sort'=>0);
		$show_data[] = array('fields_text'=>'mobile','title'=>'手机号','sort'=>0);
		$users_fields_data = $model->field('fields_text,sort')->where(array('users_id'=>$users_id,'status'=>1))->order('sort asc')->select();
		if($users_fields_data){
			//默认字段
			$default_data = $this->get_default_fields_config();
		...
		$screen_fields_data = $screen_model->field('fields_text,sort')->where(array('users_id'=>$users_id,'status'=>1))->order('sort asc')->select();
...
//						$value = getFileUrl($value,'image');
	...
	public function get_info_text($data){
		...
		$model = M('customer');
		$map['id'] = $data['id'];
		$info = $model->field('id,name,mobile,type,follow_status,next_follow_time,users_id,person_id,remark,create_time,status,no_follow_day')->where($map)->find();
		if($info){
			//用户配置
			$users_data = M('users')->field('id,name')->select();
			$users_data = array_column($users_data,'name','id');
			...
			$lists[2]['data'][] = array('title'=>'未跟进天数','content'=>$info['no_follow_day'].'天');
	...
	public function get_customer_logs($data){
		...
		if($lists){
			$type_configs = D('Setting')->get_follow_type_config();
			$type_configs = array_column($type_configs,'title','id');
			foreach ($lists as $key => $value) {
				$value['create_time'] = date('Y-m-d H:i:s',$value['create_time']);
				if(in_array($value['type'],[1,2])){
					$value['content'] = json_decode($value['content'],true);
				}
				$value['pic_type'] = $value['type'];
				switch($value['type']){
					case 1:
						$follow_type = $value['content']['follow_type'];
						$value['pic_type'] .= '_'.$follow_type;
						$title = $type_configs[$follow_type];
						break;
					case 2:
						$title = '编辑客户';
						break;
	...
	public function add_customer_logs($data){
		$data = set_trim($data);
		if(empty($data['customer_id']) || empty($data['users_id']) || empty($data['type']) || !in_array($data['type'],[1,2,3,4,5,6,7])){
			return ['msg'=>'数据不存在','status'=>400];
		}
...
//			}
			...
			$edit_data['new_follow_time'] = NOW_TIME;
	...
	public function get_fields_show($data){
		...
		$lists = $model->field('fields_text,sort,status')->where(array('users_id'=>$data['users_id']))->order('sort asc')->select();
		//默认排序
		$default_data = $this->get_default_fields_config();
		if(empty($lists)){
			$lists = $default_data;
	...
	public function add_follow($data){
		...
		//修改客户跟进状态
		$save_customer_data['new_follow_time'] = NOW_TIME;
		$save_customer_data['no_follow_day'] = 0;
		...
		$log_data_content['follow_type'] = $data['type'];
		if($pics){
	 ...
	 * 转移至公海
	 */
	public function into_waters($data){
		if(empty($data['users_id']) || empty($data['id'])){
			return ['msg'=>'数据不存在','status'=>400];
		}
		$ids = $data['id'];
		...
		//开启事务
		$model->startTrans();
		$ids = $model->where(array('id'=>['in',$ids],'status'=>1))->getField('id',true);
		if(empty($ids)){
			return ['msg'=>'数据不存在','status'=>400];
		}
		foreach ($ids as $value) {
			//转移至公海
			$check = $model->where(array('id'=>$value,'status'=>1))->save(array('status'=>2,'person_id'=>0));
			if(empty($check)){
				continue;
			}
			//增加日志
			...
			$log_data = array();
			$log_data['users_id'] = $data['users_id'];
			$log_data['customer_id'] = $value;
			$log_data['type'] = 5;
			$log_data['content'] = '客户被'.$users_name.'转入公海';
			$res = $this->add_customer_logs($log_data);
			if($res['status'] != 200){
				$model->rollback();
				return ['msg'=>'转移失败','status'=>400];
